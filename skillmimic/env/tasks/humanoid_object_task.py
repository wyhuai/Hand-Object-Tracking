...
class HumanoidWholeBodyWithObject(HumanoidWholeBody): #metric
    ...
    def extract_keypoints(self, env_ids=None, motid=None):
        ...
        if self.body_to_obj_keypoint:
            ...
            if motid is not None:
                ...
                ref_tar_pos = last_frame_ref_obs[:, 109:112]  # shape: [B, 3]
                ref_tar_rot = last_frame_ref_obs[:, 112:116]  # shape: [B, 4]
    ...
    def _build_traj(self, env_id, env_ptr):
        ...
        for i in range(16):
            # build object traj
            tmp_col_group = self.num_envs*2 + env_id*16 + i # 互相不碰撞
            traj_pose = gymapi.Transform()
            traj_pose.p.x = 0
            traj_pose.p.y = 0
            traj_pose.p.z = 1
    ...
    def _build_traj_tensors(self):
        ...
        self._keypose_traj_states = self._root_states.view(self.num_envs, num_actors, self._root_states.shape[-1])[..., 4:4+16, :]
