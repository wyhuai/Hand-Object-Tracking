...
class ObjectModel:
    ...
    def __init__(self, data_root_path, batch_size_each, num_samples=2000, device="cuda"):
        ...
        #self.scale_choice = torch.tensor([0.06, 0.08, 0.1], dtype=torch.float, device=self.device) #usb 0.05
        self.scale_choice = torch.tensor([0.2], dtype=torch.float, device=self.device)
