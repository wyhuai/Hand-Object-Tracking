...
class SkillMimicBallPlay(HumanoidWholeBodyWithObjectPlane): 
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.postproc_hopdata = cfg['env']['postproc_hopdata']
        ...
        if self.hand_model == "mano":
            ...
            self.ref_hoi_obs_size = 119 + (len(cfg["env"]["keyBodies"]))*3 +2
            ...
            self.ref_hoi_obs_size = 73 + (len(cfg["env"]["keyBodies"]))*3+2
            ...
            self.ref_hoi_obs_size = 61 + (len(cfg["env"]["keyBodies"]))*3+2 
    ...
    def _compute_observations(self, env_ids=None): # called @ reset & post step
        ...
        obs = torch.cat([humanoid_obs, obj_obs], dim=-1)
    ...
    def _compute_reset(self):
        ...
        self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_reset(self.reset_buf, self.progress_buf, 
                                                                            self._contact_forces, self.max_episode_length,
                                                                            self._enable_early_termination, self._motion_data.envid2episode_lengths,
    ...
    def _compute_metrics(self):
        ...
        self.extras["metrics"]['E_op'], self.extras["metrics"]['E_or'], self.extras["metrics"]['E_h'], self.extras["metrics"]['error_done'] = \
            compute_metrics(self._curr_ref_obs, self._curr_obs, len(self._key_body_ids), self._ref_target_keypoints_per_epoch, self._obs_target_keypoints_per_epoch, self.hand_model)
        return
    ...
    def _compute_hoi_observations(self, env_ids=None):
        ...
        self._curr_obs[env_ids] = build_hoi_observations(self._rigid_body_pos[indexer][:, 0, :].clone(), 
                                                         self._rigid_body_rot[indexer][:, 0, :].clone(), 
                                                         self._dof_pos[indexer].clone(), 
                                                         self._dof_vel[indexer].clone(), 
                                                         key_body_pos[indexer].clone(), 
    ...
    def play_dataset_postproc_hopdata(self): #Z12
    ...
    def play_dataset_step(self, time, motid=None, length=None): #Z12
        ...
        t = time % self._motion_data.motion_lengths[motid]
        if time == 3:
    ...
    def render(self, sync_frame_time=False, t=0):
        ...
        if self.viewer:
            ...
            if self.save_images:
                env_ids = 0
                os.makedirs("hot/data/images/" + self.save_images_timestamp, exist_ok=True)
                frame_id = t if self.play_dataset else self.progress_buf[env_ids]
                frame_id = len(os.listdir("hot/data/images/" + self.save_images_timestamp))
                rgb_filename = "hot/data/images/" + self.save_images_timestamp + "/rgb_env%d_frame%05d.png" % (env_ids, frame_id)
...
@torch.jit.script
def build_hoi_observations(root_pos, root_rot, dof_pos, dof_vel, key_body_pos, target_states, progress_buf):
    num_envs = key_body_pos.shape[0]
    dof_vel = dof_vel*(progress_buf!=1).unsqueeze(dim=-1)
    contact1 = torch.zeros(num_envs,1,device=dof_vel.device)
    contact2 = torch.zeros(num_envs,1,device=dof_vel.device)
    obs = torch.cat((root_pos, root_rot, \
                     dof_pos, dof_vel, \
                     target_states[:,:10], \
                     key_body_pos.contiguous().view(num_envs,-1), \
                     contact1, contact2), dim=-1)
    return obs
...
def compute_humanoid_reward(hoi_ref: Tensor, hoi_obs: Tensor, hoi_obs_hist: Tensor,
                            contact_buf: Tensor, tar_contact_forces: Tensor, len_keypos: int, 
                            w: Dict[str, Tensor], skill_label: Tensor, enable_ig_scale: bool, 
                            ref_obj_keypoints: Tensor,
                            obs_obj_keypoints: Tensor, 
                            hand_model) -> Tensor:
    ...
    ewr = torch.acos(dot) / torch.pi 
    rwr = torch.exp(-ewr*w['wr'])

    ewpv = torch.mean((ref_wrist_pos_vel - wrist_pos_vel)**2,dim=-1)
    rwpv = torch.exp(-ewpv*w['wpv'])
...
# @torch.jit.script
def compute_metrics(hoi_ref: Tensor, hoi_obs: Tensor,len_keypos: int, ref_obj_keypoints: Tensor,
                            obs_obj_keypoints: Tensor, hand_model="mano") -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    if hand_model=="mano":
        obj_pos = hoi_obs[:,109:109+3]
        obj_rot = hoi_obs[:,112:112+4]
        key_pos = hoi_obs[:,119:119+len_keypos*3]
    ...
    dot = torch.sum(ref_norm * obj_norm, dim=-1) # dot product
    ...
    angle_rad = torch.abs(angle_rad) * 180 / torch.pi # radians to degrees
...
def compute_humanoid_reset(reset_buf, progress_buf, contact_buf, 
                           max_episode_length, enable_early_termination, envid2episode_lengths,
                           isTest, maxEpisodeLength, E_op, E_or, E_h):
    # type: (Tensor, Tensor, Tensor, float, bool, Tensor, bool, int, Tensor, Tensor, Tensor) -> Tuple[Tensor, Tensor]
    terminated = torch.zeros_like(reset_buf)

    if enable_early_termination:
        # if ObjPosError > 10, or ObjRotError > 90, or KeyPosError > 20, terminate the env
        has_failed_nocontact = ((E_op>10) | (E_or>90) | (E_h>20)) & (progress_buf>5)
