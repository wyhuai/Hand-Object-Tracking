...
# import torch
# from torch_utils import exp_map_to_quat # 假设torch_utils已定义
# # 需要确保smooth_quat_seq, generate_manipulation_trajectory, generate_release_wrist_trajectory等函数已定义

def load_grasp_traj(seq_path, device='cpu', traj_length=20):
    """
    加载抓取轨迹并进行预处理。
    ...
    'contact': graspxl_data[:, 110:111].clone()
    }

    print("抓取轨迹加载完成。")
    return grasp_traj

def generate_manipulation_traj(current_traj, target_obj_pose, num_steps=50):
    """
    生成操作轨迹，可以多次调用以生成多段轨迹。

    参数:
    current_traj (dict): 当前完整的轨迹字典。
    target_obj_pose (tuple): 目标物体位置和旋转 (pos, rot)。
    num_steps (int): 插值步数，默认为50。
    ...
    # 获取当前轨迹的末端状态
    init_obj_pos = current_traj['obj_pos'][-1].clone() # 全局位置
    init_obj_rot = current_traj['obj_rot'][-1].clone() # 全局四元数
    init_wrist_pos = current_traj['wrist_dof'][-1, :3].clone() # 局部位置
    init_wrist_rot = current_traj['wrist_dof'][-1, 3:].clone() # 局部欧拉角
    init_root_pos = current_traj['root_pos'][-1].clone() # 全局位置
    init_root_rot = current_traj['root_rot'][-1].clone() # 全局四元数

    target_obj_pos, target_obj_rot = target_obj_pose

    # 生成操作轨迹
    obj_pos, obj_rot, wrist_dof = generate_manipulation_trajectory(
    init_obj_pos,
    init_obj_rot,
    init_wrist_pos,
    init_wrist_rot,
    ...
    'obj_pos': obj_pos,
    'obj_rot': obj_rot,
    'wrist_dof': wrist_dof.float(),
    'root_pos': current_traj['root_pos'][-1].repeat(num_steps, 1),
    'root_rot': current_traj['root_rot'][-1].repeat(num_steps, 1),
    'fingers_dof': current_traj['fingers_dof'][-1].repeat(num_steps, 1),
    'contact': torch.ones(num_steps, 1)
    }

    print("操作轨迹生成完成。")
    return manipulation_traj

def generate_release_traj(grasp_traj, manipulation_traj):
    """
    生成释放轨迹。

    参数:
    grasp_traj (dict): 抓取轨迹字典。
    manipulation_traj (dict): 操作轨迹字典。
    ...
    release_traj = {
    'wrist_dof': torch.flip(grasp_traj['wrist_dof'], dims=[0]),
    'root_pos': torch.flip(grasp_traj['root_pos'], dims=[0]),
    'root_rot': torch.flip(grasp_traj['root_rot'], dims=[0]),
    'fingers_dof': torch.flip(grasp_traj['fingers_dof'], dims=[0]),
    'contact': torch.flip(grasp_traj['contact'], dims=[0])
    }

    target_obj_pos = manipulation_traj['obj_pos'][-1].clone() # 全局位置
    target_obj_rot = manipulation_traj['obj_rot'][-1].clone() # 全局四元数
    target_wrist_pos = manipulation_traj['wrist_dof'][-1, :3].clone() # 局部位置
    target_wrist_rot = manipulation_traj['wrist_dof'][-1, 3:].clone() # 局部欧拉角
    init_root_pos = manipulation_traj['root_pos'][-1].clone() # 全局位置
    init_root_rot = manipulation_traj['root_rot'][-1].clone() # 全局四元数

    len_release_traj = release_traj['fingers_dof'].shape[0]
    release_traj['obj_pos'] = target_obj_pos.repeat(len_release_traj, 1)
    release_traj['obj_rot'] = target_obj_rot.repeat(len_release_traj, 1)

    release_traj['wrist_dof'] = generate_release_wrist_trajectory(
    target_wrist_pos.float(),
    target_wrist_rot.float(),
    release_traj['wrist_dof']
    )
...
if __name__ == "__main__":
    ...
    # robot root is around 1., 0., 0.6
    N_traj = 10

    x_positions = torch.linspace(0,2,N_traj)
    y_positions = torch.linspace(0,0,N_traj)
    z_positions = torch.linspace(0.1,0.1,N_traj)

    x_angles = torch.linspace(0,0,N_traj)
    y_angles = torch.linspace(0,0,N_traj)
    z_angles = torch.linspace(0,torch.pi/2,N_traj)

    positions = torch.stack((x_positions,y_positions,z_positions),dim=1)
    rots = torch.stack((x_angles,y_angles,z_angles),dim=1)

    for idx in range(N_traj):

        # 加载抓取轨迹
        seq_path = "/home/hkust/yinhuai/skillmimic_hand/skillmimic/data/motions/box02_new/box02-1.pt"
        grasp_traj = load_grasp_traj(seq_path, device='cpu', traj_length=20)

        # 初始化完整轨迹
        full_traj = grasp_traj.copy()

        final_obj_pos = positions[idx].clone()
        final_obj_rot = torch_utils.exp_map_to_quat(rots[idx])

        # 生成多个操作轨迹
        num_manipulations = 3 # 例如，生成两段操作轨迹
        for i in range(num_manipulations):
            # # 生成随机目标物体姿态进行测试
            # target_obj_pos = (torch.rand(3) - 0.5) * 2
            # target_obj_pos[2] = 1.
            # target_obj_rot = torch_utils.exp_map_to_quat((torch.rand(3) - 0.5) * torch.pi/2 *0)
            # target_obj_pose = (target_obj_pos, target_obj_rot)

            # final_obj_pos = torch.tensor([1.,1.,0.1]) # on the floor
            # final_obj_rot = torch_utils.exp_map_to_quat(torch.tensor([0.,0.,-torch.pi/3]))
            if i == 0: 
                target_obj_pos = full_traj['obj_pos'][-1].clone()
                target_obj_pos[2] = 1.
                target_obj_rot = full_traj['obj_rot'][-1].clone()
                target_obj_rot[0] = final_obj_rot[0].clone()
                target_obj_rot[1] = final_obj_rot[1].clone()
                target_obj_rot[2] = final_obj_rot[2].clone()
                target_obj_rot[3] = final_obj_rot[3].clone()
                target_obj_pose = (target_obj_pos, target_obj_rot)
            if i == 1: 
                target_obj_pos = full_traj['obj_pos'][-1].clone()
                target_obj_pos[0] = final_obj_pos[0].clone()
                target_obj_pos[1] = final_obj_pos[1].clone()
                target_obj_pos[2] = 1.
                target_obj_rot = full_traj['obj_rot'][-1].clone()
                target_obj_rot[0] = final_obj_rot[0].clone()
                target_obj_rot[1] = final_obj_rot[1].clone()
            ...
            if i == 2: 
                ...
                target_obj_pos[0] = final_obj_pos[0].clone()
                target_obj_pos[1] = final_obj_pos[1].clone()
                target_obj_pos[2] = final_obj_pos[2].clone()
                target_obj_rot = full_traj['obj_rot'][-1].clone()
                target_obj_rot[0] = final_obj_rot[0].clone()
                target_obj_rot[1] = final_obj_rot[1].clone()
                target_obj_rot[2] = final_obj_rot[2].clone()
            ...
            manipulation_traj = generate_manipulation_traj(full_traj, target_obj_pose, num_steps=25)

            # 连接轨迹
            full_traj['obj_pos'] = torch.cat((full_traj['obj_pos'], manipulation_traj['obj_pos']), dim=0)
            full_traj['obj_rot'] = torch.cat((full_traj['obj_rot'], manipulation_traj['obj_rot']), dim=0)
            full_traj['wrist_dof'] = torch.cat((full_traj['wrist_dof'], manipulation_traj['wrist_dof']), dim=0)
            full_traj['root_pos'] = torch.cat((full_traj['root_pos'], manipulation_traj['root_pos']), dim=0)
            full_traj['root_rot'] = torch.cat((full_traj['root_rot'], manipulation_traj['root_rot']), dim=0)
            full_traj['fingers_dof'] = torch.cat((full_traj['fingers_dof'], manipulation_traj['fingers_dof']), dim=0)
            full_traj['contact'] = torch.cat((full_traj['contact'], manipulation_traj['contact']), dim=0)

        # 生成释放轨迹
        release_traj = generate_release_traj(grasp_traj, manipulation_traj)

        # 连接释放轨迹
        full_traj['obj_pos'] = torch.cat((full_traj['obj_pos'], release_traj['obj_pos']), dim=0)
        full_traj['obj_rot'] = torch.cat((full_traj['obj_rot'], release_traj['obj_rot']), dim=0)
        full_traj['wrist_dof'] = torch.cat((full_traj['wrist_dof'], release_traj['wrist_dof']), dim=0)
        full_traj['root_pos'] = torch.cat((full_traj['root_pos'], release_traj['root_pos']), dim=0)
        full_traj['root_rot'] = torch.cat((full_traj['root_rot'], release_traj['root_rot']), dim=0)
        full_traj['fingers_dof'] = torch.cat((full_traj['fingers_dof'], release_traj['fingers_dof']), dim=0)
        full_traj['contact'] = torch.cat((full_traj['contact'], release_traj['contact']), dim=0)

        # 拼接最终轨迹数据
        len_full_traj = full_traj['obj_pos'].shape[0]

        final_obj_pos[-1] = final_obj_pos[-1]/2
        full_traj['obj2_pos'] = final_obj_pos.clone().repeat(len_full_traj,1)
        full_traj['obj2_rot'] = final_obj_rot.clone().repeat(len_full_traj,1)
        ...
        # 保存最终轨迹数据
        save_path = f'/home/hkust/yinhuai/skillmimic_hand/skillmimic/data/motions/box02_assembly/box02_{idx}.pt' #graspmimic/Camera_gen_release2/camera.pt
