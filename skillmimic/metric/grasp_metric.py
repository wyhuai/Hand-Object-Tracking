...
class GraspMetric(BaseMetric):
    def __init__(self, num_envs, device, h_offset, max_episode_length):
        super().__init__(num_envs, device, h_offset)
        self.at_target = torch.zeros((num_envs, 180), device=device, dtype=torch.bool)  # Assuming max progress_buf = 500
        self.reached_target = torch.zeros(num_envs, device=device, dtype=torch.bool)

    def reset(self, env_ids):
        self.at_target = torch.zeros((self.num_envs, 180), device=self.device, dtype=torch.bool)
    ...
    def update(self, state):
        ...
        if current_step >= 50:
            threshold = 0.1 + self.h_offset
            at_target = (distance_obj2wrist < 0.3) & (obj_pos_vel[:, 0] < 0.1) & (obj_pos_vel[:, 1] < 0.1) & (obj_pos[:, 2] >= threshold)
        else:
            at_target = torch.norm(obj_pos_vel, dim=-1) <= 0.2
        self.at_target[:, current_step] = at_target
        self.reached_target = torch.sum(self.at_target, dim=-1) > 120
