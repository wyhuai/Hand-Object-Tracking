...
class FindStablePose:
    ...
    def compute_pose(self):
        ...
        random_quats = self.random_quaternion(self.num_envs)
        ...
        for i in range(1000): 
                ...
                if i == 200: # i=200 when most object stablise on the ground                 
                    ...
                    for e in range(1, self.num_envs):
                        ...
                        # Save image 
                        if find_match == None and self.save_images:
                            image_dir = "../../data/free_drop_image/" + self.object_name + "/gp_" + str(num_keys)
                            os.makedirs(image_dir)
                            rgb_filename = image_dir + "/%s_env%d.png" % (self.object_name, e)
                            self.gym.write_viewer_image_to_file(self.viewer,rgb_filename)
                        elif(len(rotation_groups[key]) <= 10) and self.save_images: # only save when a group has less then 10 photos
                            image_dir = "../../data/free_drop_image/" + self.object_name + "/gp_" + str(find_match)
                            rgb_filename = image_dir + "/%s_env%d.png" % (self.object_name, e)
                            self.gym.write_viewer_image_to_file(self.viewer,rgb_filename)

                    self.save_first_quats(rotation_groups, object_root_states, f"../../data/free_drop_image/{self.object_name}/oldunique_quats.txt")
                ...
                if self.save_images and i == 900: 
                    ...
                    for key in unique_rotation_angle:
                        ...
                        if (self.quat_equal(object_root_states[key, 3:7], unique_rotation_angle[key].clone())):
                            ...
                            if self.save_images:
                                self.gym.draw_viewer(self.viewer, self.sim, True)
                                self.gym.sync_frame_time(self.sim)
                                image_dir = "../../data/free_drop_image/" + self.object_name + "/final"
                    ...
                    self.save_unique_quats(final_quat, f"../../data/free_drop_image/{self.object_name}/unique_quats.txt")
...
if __name__ == "__main__":
    processor = FindStablePose(asset_path = "../../data/assets/urdf/Box/Box.urdf")
